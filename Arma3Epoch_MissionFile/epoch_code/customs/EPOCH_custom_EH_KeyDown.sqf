...
if (vehicle player == player) then {
	...
	if (EPOCH_buildMode > 0) then {
		if (!_ctrl) then {
			...
			case EPOCH_keysBuildMovBak: { _adj = 0.1;if(_shift)then{_adj = 0.5};if(_alt)then{_adj = 0.01};EPOCH_Y_OFFSET = (EPOCH_Y_OFFSET - _adj) max 2; _handled = true };
				...
				if (EPOCH_target_attachedTo isEqualTo player) then {
					[]spawn{
						...
						if (_result) then
						{
							...
							EPOCH_doRotate = true;
							_msg = format["Object Snapping : ON  Snapped To : %1",typeOf EPOCH_target_attachedTo];
